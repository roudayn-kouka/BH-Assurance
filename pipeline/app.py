...
PP_FILE = os.getenv(
    "PP_FILE", "../datasets/catboost_produits_pp_ranked_final_clean.csv"
)
PM_FILE = os.getenv(
    "PM_FILE", "../datasets/catboost_produits_pm_ranked_final_clean.csv"
)
...
def build_recommended_products(
    filtered_df: pd.DataFrame, product_col: str, score_col: str
) -> List[Dict]:
    records = []
...
def get_random_user():
    ...
    if not dataset:
        raise HTTPException(
            status_code=404, detail="Aucun utilisateur disponible dans les datasets"
        )
    ...
    try:
        if dataset == "pp":
            ...
            filtered = user_subset[
                user_subset["Candidate_Produit"] != user_subset["Lib_Produit"]
            ]
            recommended = build_recommended_products(
                filtered, "Candidate_Produit", "score"
            )
            ...
            owned_products = build_all_products(user_subset, "LIB_PRODUIT")
            filtered = user_subset[
                user_subset["candidate_product"] != user_subset["LIB_PRODUIT"]
            ]
            recommended = build_recommended_products(
                filtered, "candidate_product", "score"
            )
...
def get_user_by_id(user_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Utilisateur avec ID {user_id} introuvable"
        )
