...
def initialize_classifier():
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            "joeddav/xlm-roberta-large-xnli", use_fast=False, trust_remote_code=False
        )
...
def analyse_message(text, threshold=0.7):
    ...
    for intent, (pattern, score) in PATTERNS.items():
        if re.search(pattern, txt):
            ...
            return {
                ...
                "produit": {
                    "label": produits[0] if produits else "aucun",
                    "score": 1.0 if produits else 0.0,
                },
            }
    ...
    if model_available and classifier is not None:
        try:
            result = classifier(
                text, INTENT_LABELS, hypothesis_template="Ce client exprime {}."
            )
            ...
            if intent == "intérêt spécifique" and intent_score >= threshold:
                ...
                produit, produit_score = product_result["labels"][0], float(
                    product_result["scores"][0]
                )
        except Exception as e:
